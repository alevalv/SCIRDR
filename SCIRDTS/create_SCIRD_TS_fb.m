function [SCIRD_TS_filter_bank, properties, all_ind] = create_SCIRD_TS_fb(sigma_1,sigma_1_step, sigma_2,sigma_2_step, k, k_step, step_angle, filter_size)

% create_SCIRD_TS_fb creates a SCIRD-TS filter bank
    ...
    for i_s2 = sigma_2(1):sigma_2_step:sigma_2(end)
        ...
        for i_k = k(1)-1e-15:k_step:k(end)+1e-15
            if i_s1>=i_s2 %filters should be longer than wider
                ...
                %create a grid
                Sigma = max(i_s1,i_s2);
                [x,y]   = ndgrid([-floor(filter_size/2):0,1:floor(filter_size/2)]);
